...
namespace denizdikbiyik_CET322_FinalProject.Areas.Identity.Pages.Account.Manage
{
    public class ExternalLoginsModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetLinkLoginCallbackAsync()
        {
            ...
            if (info == null)
            {
                throw new InvalidOperationException($"Şu kullanıcı için dış giriş yüklenirken bilinmeyen bir hata oluştu: '{user.Id}'.");
            }

            var result = await _userManager.AddLoginAsync(user, info);
            if (!result.Succeeded)
            {
                throw new InvalidOperationException($"Şu kullanıcı için dış giriş eklenirken bilinmeyen bir hata oluştu: '{user.Id}'.");
            }
